...
if __name__ == "__main__":
    ...
    # FFT plot for magnitude of acceleration (sqrt(ax^2 + ay^2)), raw and filtered
    def plot_and_save_fft_mag(ax, ay, ax_filt, ay_filt, fs):
        raw_mag = np.sqrt(ax**2 + ay**2)
        filt_mag = np.sqrt(ax_filt**2 + ay_filt**2)
        N = len(raw_mag)
        freq = np.fft.rfftfreq(N, d=1/fs)
        fft_orig = np.abs(np.fft.rfft(raw_mag))
        fft_filt = np.abs(np.fft.rfft(filt_mag))

        plt.figure(figsize=(8,4))
        plt.plot(freq, fft_orig, label='Raw Magnitude')
        plt.plot(freq, fft_filt, label='Filtered Magnitude')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Magnitude')
        plt.title('FFT of Acceleration Magnitude (sqrt(ax² + ay²))')
        plt.legend()
        plt.xlim(-5, 10)
        plt.tight_layout()
        plt.savefig('fft_accel_mag_xy.png')
        plt.close()

    # Only for magnitude of acceleration (x and y)
    plot_and_save_fft_mag(df['ax'], df['ay'], df['ax_filtered'], df['ay_filtered'], sampling_rate)
